...
public class PictureUploadService {
    ...
    public PresignPutResponse presignPut(UUID bucketRootKey, PresignPutRequest request) {
        if (request.fileSize() > MAX_FILE_SIZE) {
            throw new GlobalException(ErrorCode.FILE_SIZE_EXCEEDED);
        }

        String bucket = awsProps.bucket();
        String key = s3KeyFactory.postImageKey(boardId, request.fileExt());
        ...
        return new PresignPutResponse(uploadUrl, key, publicUrl);
    }
}
